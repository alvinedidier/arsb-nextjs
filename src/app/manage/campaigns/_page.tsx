    ...
    <h1 className="text-2xl font-bold mb-4">Liste des campagnes</h1>
      ...
      <table className="min-w-full bg-white">
        ...
        <tbody>
          {data.map(campaign => (
            <tr key={campaign.id}>
              <td className="py-2 px-4 border-b"><Link href={`/manage/campaigns/${campaign.id}`}>{campaign.id}</Link></td>
              <td className="py-2 px-4 border-b"><Link href={`/manage/campaigns/${campaign.id}`}>{campaign.name}</Link></td>
